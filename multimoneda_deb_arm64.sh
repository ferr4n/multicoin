...
# TraffMonetizer, HoneyGain, EarnApp, Pawns/IPRoyal, PacketStream, RePocket, Proxyrack, ProxyLite, Mysterium, EarnFM, SpeedShare, Filecoin Station y BitPing.
...
#echo "Instalar (o reinstalar) y desinstalar aplicaciones TraffMonetizer, HoneyGain, EarnApp, Pawns/IPRoyal, PacketStream, RePocket, Proxyrack, ProxyLite, Mysterium, EarnFM, SpeedShare, Filecoin Station, Grass, Streamr y BitPing"
echo "Instalar (o reinstalar) y desinstalar aplicaciones TraffMonetizer, HoneyGain, EarnApp, Pawns/IPRoyal, PacketStream, RePocket, Proxyrack, ProxyLite, Mysterium, EarnFM, SpeedShare, Filecoin Station, Grass, Meson y BitPing"
echo
...
echo
# Descomente para Streamr (experto):
#APPS=`docker ps -a --format '{{.Names}}' | grep -F -e traffmonetizer -e honeygain -e pawns -e packetstream -e repocket -e proxyrack -e proxylite -e mysterium -e earnfm -e filecoinstation -e bitping -e streamr -e grass | tee /dev/tty`
APPS=`docker ps -a --format '{{.Names}}' | grep -F -e traffmonetizer -e honeygain -e pawns -e packetstream -e repocket -e proxyrack -e proxylite -e mysterium -e earnfm -e filecoinstation -e bitping -e grass | tee /dev/tty`
...
if [[ $instmmin = "si" ]]; then
 ...
 echo Instalando imagen de docker traffmonetizer con el token $tokentm y actualizador updaterTM...
 ...
 echo TraffMonetizer instalado y funcionando.
else
 if [[ "$APPS" =~ .*"traffmonetizer".* ]]; then
  ...
  if [[ $desinstmmin = "si" ]]; then
   echo Desinstalando imagen de docker traffmonetizer y actualizador updaterTM...
   docker stop traffmonetizer >> $LOG 2>&1
   docker rm traffmonetizer >> $LOG 2>&1
...
if [[ $inshgmin = "si" ]]; then
 ...
 echo Instalando imagen de docker honeygain con el email $emailhg y la clave $passhg y actualizador updaterHG...
 ...
 echo HoneyGain instalado y funcionando.
else
 if [[ "$APPS" =~ .*"honeygain".* ]]; then
  echo "¿Quiere eliminarlo completamente? [si/NO] :"
  read desinshg
  ...
  if [[ $desinshgmin = "si" ]]; then
   echo Desinstalando imagen de docker honeygain y actualizador updaterHG...
   docker stop honeygain >> $LOG 2>&1
   docker rm honeygain >> $LOG 2>&1
...
if [[ $inspawnsmin = "si" ]]; then
 ...
 echo Instalando imagen de docker pawns con el email $emailpawns y la clave $passpawns y actualizador updaterPawns...
 ...
 echo Pawns - IP Royal instalado y funcionando.
else
 echo "¿Quiere eliminarlo completamente? [si/NO] :"
 read desinspawns
 desinspawnsmin=$(echo $desinspawns | tr '[:upper:]' '[:lower:]')
 if [[ $desinspawnsmin = "si" ]]; then
  echo Desinstalando imagen de docker pawns y actualizador updaterPawns...
  docker stop pawns >> $LOG 2>&1
  docker rm pawns >> $LOG 2>&1
...
if [[ $insmymin = "si" ]]; then
 ...
 echo Cuando lo haya hecho pulse enter para continuar.
 read continuar
 echo Instalando la imagen de docker mysterium y el actualizador updaterMyst...
 ...
 echo Cuando lo haya hecho pulse enter para continuar.
 read continuar
 echo Mysterium instalado y funcionando.
else
 if [[ "$APPS" =~ .*"mysterium".* ]]; then
  echo "¿Quiere eliminarlo completamente? [si/NO] :"
  read desinsmy
  ...
  if [[ $desinsmymin = "si" ]]; then
   echo Desinstalando imagen de docker mysterium y actualizador updaterMyst...
   docker stop mysterium >> $LOG 2>&1
   docker rm mysterium >> $LOG 2>&1
...
## Streamr: NO LO INSTALAMOS POR NO ESTAR BIEN PROBADO
#echo
